...
const isEdit = !!props.dosen;

const form = useForm({
    nidn_nik: props.dosen?.nidn_nik || '',
    nama: props.dosen?.nama || '',
    foto: props.dosen?.foto || '',
    foto_file: null,
    tempat_lahir: props.dosen?.tempat_lahir || '',
    tanggal_lahir: props.dosen?.tanggal_lahir || '',
    pendidikan: props.dosen?.pendidikan || '',
    homebase_prodi: props.dosen?.homebase_prodi || '',
    alamat: props.dosen?.alamat || '',
    nik_ktp: props.dosen?.nik_ktp || '',
    kepemilikan_rek_bri: props.dosen?.kepemilikan_rek_bri || '',
    status: props.dosen?.status || 'Aktif',
});

const fotoMode = ref(props.dosen?.foto && props.dosen.foto.startsWith('/storage/') ? 'upload' : 'link');
const fotoPreview = ref(null);

const fotoSrc = computed(() => {
    if (fotoPreview.value) return fotoPreview.value;
    if (fotoMode.value === 'link') return form.foto || null;
    return props.dosen?.foto || null;
});
...
const submit = () => {
    if (isEdit) {
        ...
        form.post(route('dosen.store'), { forceFormData: true });
    }
};
</script>
    ...
    <PengelolaLayout>
        ...
        <template #header>
            <div class="flex items-center gap-2 text-sm">
                <Link :href="route('dosen.index')" class="text-gray-500 hover:text-gray-700">Data Dosen</Link>
        ...
        <div class="max-w-2xl space-y-4">
            ...
            <div v-if="isEdit" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-5 py-4 flex items-center justify-between">
                    ...
                    <Link :href="route('dokumen.show', { civitasType: 'dosen', civitasId: dosen.id })"
                ...
                <div class="px-5 pb-4 grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                    <div v-for="dok in dokumen" :key="dok.key"
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h2 class="text-base font-semibold text-gray-800 mb-6">{{ isEdit ? 'Edit Data Dosen' : 'Tambah Dosen Baru' }}</h2>
                <form @submit.prevent="submit" class="space-y-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">NIDN / NIK</label>
                            <input v-model="form.nidn_nik" type="text" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#996600]/30 focus:border-[#996600]" />
                            <p v-if="form.errors.nidn_nik" class="text-red-500 text-xs mt-1">{{ form.errors.nidn_nik }}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input v-model="form.tanggal_lahir" type="text" placeholder="contoh: 9 Desember 1968" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#996600]/30 focus:border-[#996600]" />
                        </div>
                        <div>
                            ...
                            <select v-model="form.pendidikan" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#996600]/30 focus:border-[#996600]">
                                ...
                                <option>S3</option><option>S2</option><option>S1</option><option>D3</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Homebase Program Studi</label>
                            <input v-model="form.homebase_prodi" list="prodi-list" type="text" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#996600]/30 focus:border-[#996600]" />
                            <datalist id="prodi-list">
                                <option v-for="p in prodi_list" :key="p" :value="p" />
                            </datalist>
                        </div>
                        <div>
                    ...
                    <div class="flex gap-3 pt-2">
                        ...
                        <Link :href="route('dosen.index')" class="bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium px-5 py-2 rounded-lg transition-colors">Batal</Link>
                    </div>
                </form>
            </div>
        </div>
